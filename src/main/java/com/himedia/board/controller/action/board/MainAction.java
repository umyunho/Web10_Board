...
public class MainAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 조회한 레코드갯수를  Paging 객체의  totalCount 에 입력(setTotalCoutn()이용)
		paging.setTotalCount(count);  // 이때 calPaging()이 호출되서 모든 멤버변수가 계산됩니다.
		
		// 3. Paging 객체의 startNum(시작 레코드 번호)부터 displayRow(환화면당 레코드 갯수)만큼
		// 조회합니다
		ArrayList<BoardDto> list =  bdao.getAllBoard( paging );
		
		for( BoardDto bdto : list) {
			// bdto.getNum() 으로 reply 테이블에서 댓글갯수를 조회하고 결과를  bdto.setReplycnt 로 저장
