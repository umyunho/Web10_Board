...
public class MainAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1.페이지 구성요소들을 계산하기 위해 총 레코드 갯수 조회
		int count = bdao.getAllCount();
		System.out.println("레코드 갯수" + count);
		
		//2. 조회한 레코드 갯수를 paging객체의 totalCount에 입력(setTotalCount()이용)
		paging.setTotalCount(count); //이때 calPaging()가 호출되서 모든멤버 변수가 계산
		
		//3.paging객체의 staartNum(시작 레코드 번호)부터 displayRow(한화면당 레코드 갯수)만큼
		//조회
		
		ArrayList<BoardDto> list = bdao.getAllBoard(paging);
		
		for(BoardDto bdto : list) {
			//bdto.getNum(); 으로 reply테이블에서 댓글 갯수를 조회하고 결과를 bdto.setReplycnt로 저장
